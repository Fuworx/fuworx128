"use client";
import React, { useEffect, useRef, useState } from "react";
import Logo from "@/public/logo.jpg";
import { RxCross2, RxHamburgerMenu } from "react-icons/rx";
import { usePathname, useRouter } from "next/navigation";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";
import Link from "next/link";
import { ChevronDown } from "lucide-react";
import { motion } from "framer-motion";
import DeskNav from "./DeskNav";
import { ImTicket } from "react-icons/im";

function Navigation() {
  const [open, setOpen] = useState(false);
  const [isVisible, setVisiable] = useState(true);
  const [ishover, SetIsHover] = useState(false);
  const path = usePathname();

  const router = useRouter();

  const navigateToHome = () => {
    router.push("/");
  };

  const togelhoverMenu = () => {
    SetIsHover(!ishover);
  };

  useEffect(() => {
    let lastscroll = window.scrollY;
    const controllScrolling = () => {
      const currentScroll = window.scrollY;
      if (currentScroll > lastscroll && currentScroll > 50) {
        setVisiable(false);
        setOpen(false);
      } else {
        setVisiable(true);
      }

      lastscroll = currentScroll;
    };

    window.addEventListener("scroll", controllScrolling);

    return () => window.removeEventListener("scroll", controllScrolling);
  }, []);

  const toggleOpen = () => {
    setOpen(!open);
  };

  const subMenuAnimate = {
    enter: {
      opacity: 1,
      rotateX: 0,
      transition: {
        duration: 0.1,
      },
      display: "block",
    },
    exit: {
      opacity: 0,
      rotateX: -15,
      transition: {
        duration: 0.5,
      },
      display: "none",
    },
  };

  const mobileNav = useRef(null);

  const navitmes = useRef(null);
  const logo = useRef(null);

  useGSAP(() => {
    gsap.from(logo.current, {
      duration: 1,
      ease: "power2.out",
      x: -100,
    });
    gsap.from(navitmes.current, {
      duration: 1,
      delay: 0.3,
      ease: "power2.out",
      y: -100,
    });
  });

  interface subMenu {
    name: string;
    link: string;
  }

  interface NavBarItems {
    name: string;
    link: string;
    subMenu?: subMenu[];
    active: boolean;
  }

  const navbarItems: NavBarItems[] = [
    {
      name: "About",
      link: "/about",
      active: false,
      subMenu: [
        { name: "About Us", link: "/aboutUs" },
        { name: "Our Services", link: "/ourService" },
      ],
    },
    {
      name: "Services",
      link: "/services",
      subMenu: [
        { name: "Digital Engineering Service", link: "digitalService" },
        { name: "Business Service", link: "businessService" },
      ],
      active: false,
    },
    { name: "Product", link: "/product", active: false },
    { name: "Careers", link: "/careers", active: false },
  ];

  return (
    <div
      className={`w-full h-auto z-10 transition-transform duration-300 ease-linear top-0 fixed ${
        isVisible ? "translate-y-0" : "-translate-y-full"
      }`}
    >
      <div className="flex px-5 py-3 border-b border-white/20 bg-white shadow-lg backdrop-blur-md mx-auto justify-between items-center lg:px-16">
        <div ref={logo} className="flex flex-row items-center gap-2">
          <img
            src={Logo.src}
            onClick={navigateToHome}
            alt="Logo"
            className="w-9 h-9 md:w-14 md:h-14 border-none"
          />
          <div className="uppercase text-black/70 flex flex-col font-body">
            <div className="font-bold text-[#0A1F44] text-xl lg:text-2xl">
              fuworx
            </div>
            <div className="tracking-[3px]">innovations</div>
          </div>
        </div>
        <div
          ref={navitmes}
          className={`hidden md:flex gap-4 md:gap-10 text-[#0A1F44]`}
        >
          {navbarItems.map((itmes, index) => (
            <Link
              key={index}
              href={itmes.link}
              className="relative group flex items-center gap-2 text-2xl font-semibold font-head text-[#26638]"
            >
              <div className="flex cursor-pointer items-center gap-2">
                <span>{itmes.name}</span>
                {itmes.subMenu && itmes.subMenu.length > 0 && (
                  <ChevronDown className="group-hover:rotate-180 transition-all duration-200" />
                )}
              </div>
              {itmes.subMenu && (
                <div className="absolute items-start bg-white rounded-3xl left-0 top-full hidden w-[250px] py-5 group-hover:flex flex-col gap-1">
                  {itmes.subMenu.map((sub) => (
                    <Link
                      key={sub.name}
                      href={`${itmes.link}${sub.link}`}
                      className="flex text-left cursor-pointer text-xl items-center px-5 py-3 justify-center"
                    >
                      {sub.name}
                    </Link>
                  ))}
                </div>
              )}
            </Link>
          ))}
        </div>
        <div className="hidden md:block hover:scale-110 ease-linear duration-200 transition-all">
          <div className="bg-[#0A1F44] px-4 py-2 md:px-8 md:py-2 rounded-2xl">
            <div className="font-head text-[14px] md:text-[18px] font-bold">
              Contact
            </div>
          </div>
        </div>
        {open ? (
          <RxCross2
            color="black"
            onClick={toggleOpen}
            className="md:hidden ease-initial"
            size={25}
          />
        ) : (
          <RxHamburgerMenu
            color="black"
            onClick={toggleOpen}
            className="md:hidden ease-linear"
            size={25}
          />
        )}
      </div>
      <div
        ref={mobileNav}
        className={`lg:hidden ${
          open ? "block" : "hidden"
        } font-opensans font-light text-xl text-center py-8 bg-white rounded-b-2xl`}
      >
        <ul className="flex flex-col justify-center gap-5 text-right px-10 text-black">
          {navbarItems.map((item, index) => (
            <a
              key={index}
              href={`${item.link}`}
              className={`hover:text-[#002366] hover:scale-105 py-2 text-shadow-sm ease-initial duration-200 ${
                path === item.link ? "text-[#63C7B2]" : " "
              }`}
            >
              {item.name}
            </a>
          ))}
          <button className="bg-[#0A1F44] cursor-pointer text-white font-body items-center hover:scale-105 text-shadow-sm ease-initial duration-200 text-center shadow-2xl px-6 py-2 rounded-3xl">
            Contact
          </button>
        </ul>
      </div>
    </div>
  );
}

export default Navigation;
